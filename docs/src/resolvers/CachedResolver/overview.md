...
- ```Resolver.CreateContextFromString``` and ```Resolver.CreateContextFromStrings``` are implemented. The first will create a context from the provided string if it represents a valid file path, otherwise it returns the fallback context. The latter will use the first string in the provided vector. As we expose the ability to edit the context at runtime, these are primarily implemented for compatibility with different DCC requirements.
